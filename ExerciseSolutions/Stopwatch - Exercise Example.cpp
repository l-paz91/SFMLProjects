...
int main()
{
	...
	RectangleShape resetBtn({ 90, 25 });
	resetBtn.setFillColor(Color(128, 128, 128));
	resetBtn.setPosition({ 10, 100 });

	// create text for reset button
	Text resetText(font, "Reset", 16);
	resetText.setFillColor(Color::White);
	resetText.setPosition({ 20, 103 });
			...
			if (const Event::MouseButtonPressed* btnPressed = event->getIf<Event::MouseButtonPressed>())
			{
				if (btnPressed->button == Mouse::Button::Left)
				{
					...
					if (stopStartBtn.getGlobalBounds().contains(static_cast<Vector2f>(mousePos)))
					{
						...
						std::string state = isStopwatchRunning ? "\nStarted" : "\nStopped";
						std::cout << state;
					}

					// reset button
					if (resetBtn.getGlobalBounds().contains(static_cast<Vector2f>(mousePos)))
					{
						isStopwatchRunning = false;
						elapsedTime = Time::Zero;
						stopwatchText.setString("00:00:00");
